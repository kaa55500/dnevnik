...
/**
 * Дата коротко: «03.09». Восемнадцать копий одного `slice(8) + slice(5, 7)`
 * жили по экранам, и каждая правка формата означала обход всего приложения.
 */
export function dm(iso) {
  const t = String(iso ?? '');
  return t ? `${t.slice(8)}.${t.slice(5, 7)}` : '';
}
