...
function menu(box, { weekId, settings, plan, plans }) {
  ...
  // Прежняя серая строка отсюда снята: она загоралась каждый день, когда
  // бэкап не сегодняшний, и на экране, куда заходят раз в неделю, не работала
  // ни напоминанием, ни тревогой. Долг по копии теперь виден там, где его
  // застаёт работа, — на «Дне» и на экране закрытой тренировки
  // (`backup-note.js`), а дата последней копии осталась подписью плитки
  // «Данные» и строкой в самом разделе.
  void plans;
}
